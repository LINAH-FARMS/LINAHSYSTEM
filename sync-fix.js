  ...
  window.autoCompleteMissedDaily = function autoCompleteMissedDaily() {
    try {
      ...
      for (var i = 1; i <= _AUTO_DAYS_BACK; i++) {
        ...
        // 2) سجلات الهدر للوجبات الثلاث — يُكمّل بمتوسط الهدر الفعلي في الأسبوع
        var wasteMealAvg = {};
        var wasteMealCount = {};
        mealWaste.forEach(function (mw) {
          var meal = (mw.meal || '').trim();
          var w = (parseFloat(mw.wasteEng) || 0) + (parseFloat(mw.wasteWrk) || 0) + (parseFloat(mw.wasteGuests) || 0) + (parseFloat(mw.prepWaste) || 0);
          if (w > 0) {
            wasteMealAvg[meal] = (wasteMealAvg[meal] || 0) + w;
            wasteMealCount[meal] = (wasteMealCount[meal] || 0) + 1;
          }
        });
        ['فطار', 'غداء', 'عشاء'].forEach(function (meal) {
          ...
          var avgWaste = (wasteMealCount[meal] > 0) ? Math.round((wasteMealAvg[meal] / wasteMealCount[meal]) * 10) / 10 : 0;
          mealWaste.push({
            date: d, meal: meal, chef: '', ingredients: [],
            totalPrepared: 300, // تقدير تقريبي للوجبة اليومية
            wasteEng: Math.round(avgWaste * 0.3 * 10) / 10,
            wasteWrk: Math.round(avgWaste * 0.7 * 10) / 10,
            wasteGuests: 0, prepWaste: 0,
            cost: 0, responsible: '',
            engAte: 0, engTakeaway: 0, wrkAte: 0, wrkTakeaway: 0, guests: 0,
            waterAdded: 0, gasCost: 0, salaryCost: 0,
            autoGenerated: true, autoFilled: true,
            estimatedFromAvg: avgWaste > 0,
            createdAt: createdAt
          });
